...
**断点触发时的处理提示：**
- 当触发断点时，必须检查断点所在文件及断点附近的内容，结合上下文理解当前执行状态
- 应通过工具获取某一行附近的内容（如用 `read_file` 读取对应行区间、用 `vscodeOperator_hoverAtPosition` 查询符号信息），**不要自己清点换行符**来推算行号或拼凑代码内容
- 断点触发时，断点所在行的代码处于"即将执行"状态：该行尚未执行，其变量值仍是执行前的状态；判断结果时须以此为基准
